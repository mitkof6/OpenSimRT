...
MuscleOptimization::MuscleOptimization(std::string modelFile,
        const MuscleOptimization::OptimizationParameters& optimizationParameters,
        const MomentArmFunction& momentArmFunction,
        OptimizationTarget* optimizationTarget)
        : model(modelFile), target(optimizationTarget) {
    ...
    optimizer->setConvergenceTolerance(optimizationParameters.convergenceTolerance);
    ...
    if (optimizationParameters.algorithm == SimTK::OptimizerAlgorithm::InteriorPoint) {
    ...
    // logger (after target initialization to include residual)
    auto actuatorNames = getActuatorNames(model);
    actuatorNames.insert(actuatorNames.begin(), "time");
    logger = new CSVLogger(actuatorNames);
}

MuscleOptimization::Output MuscleOptimization::solve(
    ...
    //logger
    logger->addRow(input.t, parameterSeeds);
...
Vector TorqueBasedTargetLinearMuscle::extractMuscleActivations(const Vector& x) const {
    return x.elementwiseDivide(xMax)(0, nma);
}

Vector TorqueBasedTargetLinearMuscle::extractMuscleForces(const Vector& x) const {
    return x(0, nma);
}

Vector TorqueBasedTargetLinearMuscle::extractResidualForces(const Vector& x) const {
...
double TorqueBasedTargetNonLinearMuscle::setUpperBound(const OpenSim::Muscle* m){
...
void TorqueBasedTargetNonLinearMuscle::prepareForOptimization(
    ...
    fmL = getMuscleRelatedValues(*model, activeMuscleIndices,
            [&](const Muscle& m) -> double {
...
Vector TorqueBasedTargetNonLinearMuscle::extractMuscleActivations(const Vector& x) const {
    return x(0, nma);
}

Vector TorqueBasedTargetNonLinearMuscle::extractMuscleForces(const Vector& x) const {

    auto am = extractMuscleActivations(x);
    return fmMax.elementwiseMultiply(
                        am.elementwiseMultiply(fmL).elementwiseMultiply(fmV) + fmPE)
            .elementwiseMultiply(cosAlpha);
}

Vector TorqueBasedTargetNonLinearMuscle::extractResidualForces(const Vector& x) const {
    return x(nma, getNumParameters() - nma);
}

int TorqueBasedTargetNonLinearMuscle::objectiveFunc(const Vector& x,
        bool newCoefficients, Real& rP) const {
    rP = 0.0;
    for (int i = 0; i < getNumParameters(); ++i) {
        rP += pow(abs(x[i]) / xMax[i], p);
    }
...
int TorqueBasedTargetNonLinearMuscle::gradientFunc(const Vector& x,
        bool newCoefficients, Vector& gradient) const {
    for (int i = 0; i < getNumParameters(); ++i) {
        if (x[i] < 0) {
            gradient[i] = -1.0 * p * pow(abs(x[i]) / xMax[i], p - 1);
        } else {
...
int TorqueBasedTargetNonLinearMuscle::constraintFunc(const Vector& x,
        bool newCoefficients, Vector& constraints) const {
    constraints = R * extractMuscleForces(x) + extractResidualForces(x) - tau;
    return 0;
}

int TorqueBasedTargetNonLinearMuscle::constraintJacobian(const Vector& x,
        bool newCoefficients, Matrix& jac) const {
    int n = getNumConstraints();
    jac(0, 0, n, nma) = R.colScale(fmMax.elementwiseMultiply(fmL).elementwiseMultiply(fmV).elementwiseMultiply(cosAlpha));
    jac(0, nma, n, n) = 1.0;
    return 0;
}
